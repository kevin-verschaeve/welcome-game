<script>
    ...
    const shuffle = (gameRef) => gameRef.update(newGame(false));
    const restart = (gameRef) => gameRef.set(newGame());
</script>
...
<div id="main_container">
    <Doc path={`games/${params.game}`} let:data={game} let:ref={gameRef}>
        ...
        <div id="actions">
            ...
            <button class="btn waves-effect waves-light space-left" on:click={shuffle(gameRef)}>Mélanger</button>
